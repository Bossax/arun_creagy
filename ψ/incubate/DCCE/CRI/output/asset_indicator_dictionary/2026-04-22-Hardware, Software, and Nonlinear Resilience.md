...
```mermaid
flowchart LR
    A[Asset stocks] --> P[Potential service capacity]
    G[Governance processes] --> X[Activation, operation, maintenance]
    P --> X
    X --> S[Realized service flows]
    S --> R[Resilience outcomes]
    S --> T[Trust, revenue, legitimacy]
    T --> G
    A --> C[Complexity and recurrent costs]
    C -->|raises required governance threshold| X
    G -->|if below threshold| D[Underuse and degradation]
    D --> S
    D --> T
```
...
```mermaid
timeline
    title Typical trajectory of a risky asset-governance mismatch
    Appraisal : Capital expansion prioritized
    Commissioning : Asset stock rises faster than operating capability
    Early use : Utilization gaps and maintenance arrears emerge
    Stress event : Hazard, fiscal shock, or demand shock
    Threshold crossing : Abrupt service loss or ecological regime shift
    Aftermath : Revenue and legitimacy fall; recovery becomes harder
```
...
```mermaid
flowchart TD
    A[Measure asset stock and condition] --> M1[Estimate potential capacity]
    G[Measure governance robustness] --> M2[Estimate activation threshold]
    M1 --> D[Compute activation ratio and mismatch]
    M2 --> D
    D --> P[Track service performance and volatility]
    P --> E[Watch for delays, underuse, outages, backlog, erosion of trust]
    E --> S[Stress test dependencies and shocks]
    S --> I[Intervene before threshold crossing]
```
